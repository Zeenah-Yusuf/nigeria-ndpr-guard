...
const priorityConfig = {
  critical: { label: "Critical", icon: AlertTriangle, bg: "bg-destructive/10", text: "text-destructive", border: "border-destructive/20" },
  high: { label: "High", icon: AlertCircle, bg: "bg-accent/10", text: "text-accent", border: "border-accent/20" },
  medium: { label: "Medium", icon: Info, bg: "bg-primary/10", text: "text-primary", border: "border-primary/20" },
};
    ...
    <div className="space-y-5">
        ...
        return (
          <div key={priority} className="space-y-2">
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${cfg.bg} w-fit`}>
              ...
              <span className={`text-xs font-bold ${cfg.text} uppercase tracking-wider`}>{cfg.label} Priority</span>
      ...
      {items.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="font-heading font-semibold text-foreground">Looking good!</p>
          <p className="text-sm text-muted-foreground mt-1">No critical issues found. Keep up the good compliance practices.</p>
        </div>
      )}
    </div>
  );
