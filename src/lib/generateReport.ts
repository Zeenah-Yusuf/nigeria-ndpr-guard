...
export function generateReport(data: ReportData) {
  const doc = new jsPDF();
  const brandColor: [number, number, number] = [15, 118, 110];
  ...
  doc.text("NDP Act / NDPR Compliance Report", 20, 42);
  doc.text(date, 20, 52);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...brandColor);
  doc.text("Triggered Clauses (NDP Act & NDPR)", 20, 25);
  ...
  doc.setFontSize(9);
  const disclaimer = "DISCLAIMER: This report is generated by RegTrack, an AI-powered compliance simulator built by Nexus SafeSphere (NSS) in Abuja. It is intended for educational and informational purposes only and does not constitute legal advice. This tool references the Nigeria Data Protection Act 2023, NDPR 2019 Implementation Framework, and the NDP Act General Application and Implementation Directive (GAID) 2025. For formal compliance guidance, consult a licensed Data Protection Compliance Organisation (DPCO) or the Nigeria Data Protection Commission (NDPC).";
  const discLines = doc.splitTextToSize(disclaimer, pageW - 40);
  doc.text(discLines, 20, 105);
