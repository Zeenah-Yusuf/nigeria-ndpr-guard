...
export default function OrganizationDashboard() {
  ...
  // Also refresh when regulator data changes (new scans from any source)
  useEffect(() => {
    if (user) fetchBusinesses();
  }, [assessments]);
  ...
  async function fetchBusinesses() {
    ...
    for (const scan of (scans || [])) {
      ...
      let dpcoEmail = '';
      if (dpcoLink?.dpco_id) {
        const { data: dpcoProfile } = await supabase
          .from('user_profiles')
          .select('company_name')
          .eq('id', dpcoLink.dpco_id)
          .single();
        dpcoName = dpcoProfile?.company_name || t('auth.dpco');
      }
    ...
    // 2. Also add assessments from RegulatorDataService that aren't in Supabase yet
    const userAssessments = assessments.filter(a => {
      // Match by app name and check if it's the current user's org
      ...
      <main className="flex-1 pt-24 pb-12">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            ...
            <div className={`rounded-xl p-4 ${getRiskBg(avgRiskScore)}`}>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {business.pendingApprovals > 0 && (
                            <button 
                              ...
                              className="text-xs px-2 py-1 rounded-full bg-accent/10 text-accent hover:bg-accent/20 transition-colors flex items-center gap-1 disabled:opacity-50"
                            >
                              {sendingReminder === business.id ? (
                                <RefreshCw className="w-3 h-3 animate-spin" />
                              ) : (
                                <Send className="w-3 h-3" />
                              )}
                              {sendingReminder === business.id ? t('dashboard.org.sending') : t('dashboard.org.sendReminder')}
                            </button>
                          )}
                        </div>
                      ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <Link to="/demo" className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition-all group">
              ...
              <p className="text-xs text-muted-foreground">Scan a new business or application</p>
            </Link>
            <button onClick={fetchBusinesses} className="bg-card border border-border rounded-xl p-5 hover:shadow-card transition-all group text-left">
              ...
              <p className="text-xs text-muted-foreground">Update your compliance status</p>
            </button>
